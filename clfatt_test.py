...
class CLFATTKG(nn.Module):    
    def __init__(self, dropout=0.1):
        super(CLFATTKG, self).__init__()
        #### hfl/chinese-bert-wwm 
        #### for Rumour TexT
        self.bert_text = BertModel.from_pretrained('./outputs/weibo_task_domain')   
        self.bert_kg = nn.Linear(768, 128)     
        #### for Description
        self.bert_desc = BertModel.from_pretrained('./outputs/weibo_task_domain')        
        ### for USER FEATURES
	...
	## attention over combined features 
        ...
        comb_features = torch.cat([pooled_text.unsqueeze(1), pooled_desc.unsqueeze(1), user_feature.unsqueeze(1), kgs.unsqueeze(1)], dim = 1)
        attention_weights = self.attention(comb_features)
        #transformed = self.relu(self.user_drop(transformed))            
        ## combine all features
        #combined = self.transform(comb_features)  # Batch_size x (768*2 + 128) --> B X 768   
        ...
        linear_output = self.classifier(relus)
        #dropout_output = self.dropout(combined)
